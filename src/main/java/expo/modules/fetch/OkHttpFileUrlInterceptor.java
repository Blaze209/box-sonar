...
public final class OkHttpFileUrlInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws FetchAndroidContextLostException {
        ...
        if (StringsKt.startsWith$default(strRestoreFileUrl, "file:///android_asset/", false, 2, (Object) null)) {
            ...
            try {
                return new Response.Builder().request(request).protocol(Protocol.HTTP_1_1).code(200).message(SemanticAttributes.OtelStatusCodeValues.OK).body(createAssetResponseBody(context, strRemovePrefix)).build();
        ...
        return new Response.Builder().request(request).protocol(Protocol.HTTP_1_1).code(200).message(SemanticAttributes.OtelStatusCodeValues.OK).body(companion.create(bufferedSourceBuffer, createMediaType(name), file.length())).build();
    }
