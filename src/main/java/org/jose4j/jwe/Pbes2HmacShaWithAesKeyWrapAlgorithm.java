...
public class Pbes2HmacShaWithAesKeyWrapAlgorithm extends AlgorithmInfo implements KeyManagementAlgorithm {
    ...
    protected Key deriveForEncrypt(Key key, Headers headers, ProviderContext providerContext) throws JoseException {
        ...
        if (stringHeaderValue == null) {
            ...
            headers.setStringHeaderValue("p2s", base64Url.base64UrlEncode(bArrBase64UrlDecode));
        } else {
    ...
    public Key manageForDecrypt(Key key, byte[] bArr, ContentEncryptionKeyDescriptor contentEncryptionKeyDescriptor, Headers headers, ProviderContext providerContext) throws JoseException {
        return this.keyWrap.manageForDecrypt(deriveKey(key, headers.getLongHeaderValue("p2c"), new Base64Url().base64UrlDecode(headers.getStringHeaderValue("p2s")), providerContext), bArr, contentEncryptionKeyDescriptor, headers, providerContext);
    }
    ...
    public static class HmacSha256Aes128 extends Pbes2HmacShaWithAesKeyWrapAlgorithm {
        public HmacSha256Aes128() {
            super(KeyManagementAlgorithmIdentifiers.PBES2_HS256_A128KW, "HmacSHA256", new AesKeyWrapManagementAlgorithm.Aes128().setUseGeneralProviderContext());
        }
    }
