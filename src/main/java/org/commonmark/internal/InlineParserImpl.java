...
public class InlineParserImpl implements InlineParser {
    ...
    private Node parseCloseBracket() {
        ...
        if (z) {
            ...
            if (i4 > 2) {
                strSubstring = this.input.substring(i3, i4 + i3);
            } else if (!bracket.bracketAfter) {
                strSubstring = this.input.substring(bracket.index, i);
            }
            if (strSubstring != null) {
                LinkReferenceDefinition linkReferenceDefinition = this.context.getLinkReferenceDefinition(Escaping.normalizeReference(strSubstring));
    ...
    private void processDelimiters(Delimiter delimiter) {
        ...
        while (delimiter2 != null) {
            ...
            if (!delimiter2.canClose || delimiterProcessor == null) {
                ...
                boolean z2 = false;
                while (true) {
                    if (delimiter3 == null || delimiter3 == delimiter || delimiter3 == map.get(Character.valueOf(c))) {
                        z = z2;
                        z2 = false;
                        break;
                    }
