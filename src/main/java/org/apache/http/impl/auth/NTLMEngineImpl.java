...
final class NTLMEngineImpl implements NTLMEngine {
    ...
    static class Handle {
        ...
        Handle(byte[] bArr, Mode mode, boolean z) throws NTLMEngineException {
            ...
            try {
                ...
                MessageDigest md6 = NTLMEngineImpl.getMD5();
                md5.update(bArr);
                md6.update(bArr);
                if (mode == Mode.CLIENT) {
                    md5.update(NTLMEngineImpl.SIGN_MAGIC_CLIENT);
                    md6.update(NTLMEngineImpl.SEAL_MAGIC_CLIENT);
                } else {
                    md5.update(NTLMEngineImpl.SIGN_MAGIC_SERVER);
                    md6.update(NTLMEngineImpl.SEAL_MAGIC_SERVER);
                }
                this.signingKey = md5.digest();
                this.sealingKey = md6.digest();
                this.rc4 = initCipher();
            } catch (Exception e) {
                throw new NTLMEngineException(e.getMessage(), e);
            }
    ...
    static class HMACMD5 {
        ...
        HMACMD5(byte[] bArr) {
            ...
            while (i < 64) {
                this.ipad[i] = CtapException.ERR_PIN_REQUIRED;
                this.opad[i] = 92;
                i++;
            }
            this.md5.reset();
