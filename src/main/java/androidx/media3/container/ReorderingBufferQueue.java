...
public final class ReorderingBufferQueue {
    ...
    public void add(long j, ParsableByteArray parsableByteArray) {
        ...
        if (j == -9223372036854775807L || (i = this.reorderingQueueSize) == 0 || (i != -1 && this.pendingBuffers.size() >= this.reorderingQueueSize && j < ((BuffersWithTimestamp) Util.castNonNull(this.pendingBuffers.peek())).presentationTimeUs)) {
