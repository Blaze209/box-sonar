...
public final class MatrixUtils {
    ...
    public static ImmutableList<float[]> clipConvexPolygonToNdcRange(ImmutableList<float[]> immutableList) {
        Preconditions.checkArgument(immutableList.size() >= 3, "A polygon must have at least 3 vertices.");
        ImmutableList.Builder builderAddAll = new ImmutableList.Builder().addAll((Iterable) immutableList);
        float[][] fArr = NDC_CUBE;
        int length = fArr.length;
        int i = 0;
        while (i < length) {
            float[] fArr2 = fArr[i];
            ImmutableList immutableListBuild = builderAddAll.build();
    ...
    private static boolean isInsideClippingHalfSpace(float[] fArr, float[] fArr2) {
        Preconditions.checkArgument(fArr2.length == 4, "Expecting 4 plane parameters");
    ...
    public static ImmutableList<float[]> transformPoints(float[] fArr, ImmutableList<float[]> immutableList) {
        ...
        while (i < immutableList.size()) {
            ...
            float[] fArr3 = fArr;
            android.opengl.Matrix.multiplyMV(fArr2, 0, fArr3, 0, immutableList.get(i), 0);
            float f = fArr2[0];
            float f2 = fArr2[3];
            fArr2[0] = f / f2;
            fArr2[1] = fArr2[1] / f2;
