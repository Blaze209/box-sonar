...
final class OverlayShaderProgram extends BaseGlShaderProgram {
    ...
    public OverlayShaderProgram(Context context, boolean z, ImmutableList<TextureOverlay> immutableList) throws VideoFrameProcessingException {
        ...
        if (z) {
            ...
            this.hdrTypes = null;
            Preconditions.checkArgument(immutableList.size() <= 15, "OverlayShaderProgram does not support more than 15 SDR overlays in the same instance.");
        }
        ...
        try {
            ...
            for (int i2 = 1; i2 <= this.overlays.size(); i2++) {
                ...
                if (iArr != null) {
                    int i4 = iArr[i3];
                    if (i4 == 1) {
                        Preconditions.checkArgument(textureOverlay instanceof BitmapOverlay);
                        Bitmap bitmap = ((BitmapOverlay) textureOverlay).getBitmap(j);
                        Preconditions.checkArgument(bitmap.hasGainmap());
                        Gainmap gainmap = (Gainmap) Preconditions.checkNotNull(bitmap.getGainmap());
    ...
    private static String createFragmentShader(Context context, int i, int[] iArr) throws IOException {
        ...
        sb.append(Util.loadRawResource(context, R.raw.insert_overlay_fragment_shader_methods));
        if (iArr != null) {
            sb.append(Util.loadRawResource(context, R.raw.insert_ultra_hdr));
        }
        for (int i2 = 1; i2 <= i; i2++) {
