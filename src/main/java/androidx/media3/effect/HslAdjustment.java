...
public final class HslAdjustment implements GlEffect {
    ...
    public static final class Builder {
        ...
        public Builder adjustSaturation(float f) {
            Preconditions.checkArgument(-100.0f <= f && f <= 100.0f, "Can adjust the saturation by only 100 in either direction, but provided %s", Float.valueOf(f));
            this.saturationAdjustment = f;
            return this;
        }

        public Builder adjustLightness(float f) {
            Preconditions.checkArgument(-100.0f <= f && f <= 100.0f, "Can adjust the lightness by only 100 in either direction, but provided %s", Float.valueOf(f));
