...
public final class ParsableByteArray {
    ...
    public int peekCodePoint(Charset charset) {
        ...
        return iPeekCodePointAndSize != 0 ? Ints.checkedCast(iPeekCodePointAndSize >>> 8) : INVALID_CODE_POINT;
    }
    ...
    private byte peekUtf8CodeUnitSize() {
        ...
        if ((b & CtapException.ERR_EXTENSION_FIRST) == 192 && bytesLeft() >= 2 && isUtf8ContinuationByte(this.data[this.position + 1])) {
            return (byte) 2;
        }
        if ((this.data[this.position] & CtapException.ERR_VENDOR_FIRST) == 224 && bytesLeft() >= 3 && isUtf8ContinuationByte(this.data[this.position + 1]) && isUtf8ContinuationByte(this.data[this.position + 2])) {
            return (byte) 3;
        }
    ...
    private static int decodeUtf8CodeUnit(int i, int i2, int i3, int i4) {
        byte b = (byte) i3;
        return Ints.fromBytes((byte) 0, UnsignedBytes.checkedCast(((i & 7) << 2) | ((i2 & 48) >> 4)), UnsignedBytes.checkedCast(((((byte) i2) & Ascii.SI) << 4) | ((b & CtapException.ERR_UV_BLOCKED) >> 2)), UnsignedBytes.checkedCast(((b & 3) << 6) | (((byte) i4) & 63)));
    }
}
