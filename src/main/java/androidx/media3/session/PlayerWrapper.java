...
final class PlayerWrapper extends ForwardingPlayer {
    ...
    public Timeline getCurrentTimelineWithCommandCheck() {
        ...
        if (getCurrentMediaItemWithCommandCheck() != null) {
    ...
    public Player.PositionInfo createPositionInfo() {
        ...
        Preconditions.checkState(currentMediaItemIndex >= 0);
        int currentPeriodIndex = zIsCommandAvailable2 ? getCurrentPeriodIndex() : 0;
        Preconditions.checkState(currentPeriodIndex >= 0);
        if (zIsCommandAvailable2) {
            Timeline currentTimeline = getCurrentTimeline();
            if (!currentTimeline.isEmpty()) {
                Preconditions.checkState(currentMediaItemIndex < currentTimeline.getWindowCount());
                Timeline.Window window = currentTimeline.getWindow(currentMediaItemIndex, new Timeline.Window());
                Preconditions.checkState(currentPeriodIndex == Util.constrainValue(currentPeriodIndex, window.firstPeriodIndex, window.lastPeriodIndex));
            }
        }
