...
public final class DtsUtil {
    ...
    private static final int[] SAMPLE_RATE_BY_SFREQ = {-1, 8000, AacUtil.AAC_HE_V1_MAX_RATE_BYTES_PER_SECOND, 32000, -1, -1, 11025, 22050, RecorderService.AUDIO_SAMPLING_RATE, -1, -1, 12000, 24000, OpusUtil.SAMPLE_RATE, -1, -1};
    ...
    private static final int[] SAMPLE_RATE_BY_INDEX = {8000, AacUtil.AAC_HE_V1_MAX_RATE_BYTES_PER_SECOND, 32000, RecorderService.AUDIO_BIT_RATE, 128000, 22050, RecorderService.AUDIO_SAMPLING_RATE, 88200, 176400, 352800, 12000, 24000, OpusUtil.SAMPLE_RATE, 96000, DTS_MAX_RATE_BYTES_PER_SECOND, 384000};
    ...
    public static int parseDtsAudioSampleCount(byte[] bArr) {
        ...
        if (b3 != -2) {
            ...
            i2 = b2 & CtapException.ERR_UV_BLOCKED;
            return (((i2 >> 2) | i) + 1) * 32;
        }
        i = (bArr[5] & 1) << 6;
        b = bArr[4];
    ...
    public static int getDtsFrameSize(byte[] bArr) {
        ...
        if (b3 != -2) {
            ...
            i2 = (((b2 & CtapException.ERR_UV_BLOCKED) >> 2) | i3) + 1;
            z = true;
            if (z) {
                return (i2 * 16) / 14;
            }
        ...
        b = bArr[6];
        i2 = (((b & CtapException.ERR_VENDOR_FIRST) >> 4) | i) + 1;
        if (z) {
            return (i2 * 16) / 14;
        }
        return i2;
    ...
    public static DtsHeader parseDtsHdHeader(byte[] bArr) throws ParserException {
        ...
        if (bit) {
            if (bits == 0) {
                ...
                i4 = RecorderService.AUDIO_SAMPLING_RATE;
            } else {
                if (bits != 2) {
    ...
    public static DtsHeader parseDtsUhdHeader(byte[] bArr, AtomicInteger atomicInteger) throws ParserException {
        ...
        if (i3 == 0) {
            ...
            if (bits4 == 0) {
                ...
                i2 = RecorderService.AUDIO_SAMPLING_RATE;
            } else {
                if (bits4 != 2) {
