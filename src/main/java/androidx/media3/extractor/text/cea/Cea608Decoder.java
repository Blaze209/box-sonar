...
public final class Cea608Decoder extends CeaDecoder {
    ...
    private static final int[] SPECIAL_CHARACTER_SET = {174, 176, PsExtractor.PRIVATE_STREAM_1, 191, 8482, Token.COMMENT, Token.GENEXPR, 9834, 224, 32, 232, 226, 234, 238, 244, 251};
    private static final int[] SPECIAL_ES_FR_CHARACTER_SET = {193, 201, BoxCommonConstants.REQUEST_DELETE, 218, 220, 252, 8216, Token.DEBUGGER, 42, 39, 8212, 169, 8480, 8226, 8220, 8221, 192, 194, 199, 200, 202, 203, 235, 206, 207, 239, BoxCommonConstants.REQUEST_RENAME, JfifUtil.MARKER_EOI, 249, BoxCommonConstants.REQUEST_INVITE_COLLABORATORS, 171, 187};
    private static final int[] SPECIAL_PT_DE_CHARACTER_SET = {195, 227, 205, 204, 236, BoxCommonConstants.REQUEST_OPTIONS, 242, 213, 245, 123, 125, 92, 94, 95, 124, 126, 196, BoxSearchFragment.REQUEST_FILTER_SEARCH_RESULTS, 214, 246, 223, Token.ARROW, Token.METHOD, 9474, 197, 229, JfifUtil.MARKER_SOI, 248, 9484, 9488, 9492, 9496};
    ...
    private static boolean isCtrlCode(byte b) {
        return (b & CtapException.ERR_EXTENSION_FIRST) == 0;
    }

    private static boolean isExtendedWestEuropeanChar(byte b, byte b2) {
        return (b & 246) == 18 && (b2 & CtapException.ERR_EXTENSION_FIRST) == 32;
    }

    private static boolean isMidrowCtrlCode(byte b, byte b2) {
        return (b & 247) == 17 && (b2 & CtapException.ERR_VENDOR_FIRST) == 32;
    }

    private static boolean isMiscCode(byte b, byte b2) {
        return (b & 246) == 20 && (b2 & CtapException.ERR_VENDOR_FIRST) == 32;
    }

    private static boolean isPreambleAddressCode(byte b, byte b2) {
        return (b & CtapException.ERR_VENDOR_FIRST) == 16 && (b2 & 192) == 64;
    }

    private static boolean isRepeatable(byte b) {
        return (b & CtapException.ERR_VENDOR_FIRST) == 16;
    }
    ...
    private static boolean isSpecialNorthAmericanChar(byte b, byte b2) {
        return (b & 247) == 17 && (b2 & CtapException.ERR_VENDOR_FIRST) == 48;
    }
                            ...
                            if (this.isInCaptionService && updateAndVerifyCurrentChannel(b)) {
                                if (isCtrlCode(b)) {
                                    if (isSpecialNorthAmericanChar(b, b2)) {
                                        ...
                                        this.currentCueBuilder.tabOffset = b2 + CtapException.ERR_EXTENSION_FIRST;
                                    }
                                } else {
                                    this.currentCueBuilder.append(getBasicChar(b));
                                    if ((b2 & CtapException.ERR_EXTENSION_FIRST) != 0) {
