...
public class MediaCodecAudioRenderer extends MediaCodecRenderer implements MediaClock {
    ...
    protected long getDurationToProgressUs(long j, long j2, boolean z) {
        boolean z2 = this.audioSink.hasPendingData() && this.nextBufferToWritePresentationTimeUs != -9223372036854775807L;
        if (!this.isStarted) {
            return (z2 || super.isEnded()) ? 1000000L : 10000L;
        }
        long audioTrackBufferSizeUs = this.audioSink.getAudioTrackBufferSizeUs();
        if (this.hasReportedAudioPositionAdvancing && z2 && audioTrackBufferSizeUs != -9223372036854775807L) {
    ...
    protected void handleInputBufferSupplementalData(DecoderInputBuffer decoderInputBuffer) {
        if (decoderInputBuffer.format != null && Objects.equals(decoderInputBuffer.format.sampleMimeType, MimeTypes.AUDIO_OPUS) && isBypassEnabled()) {
    ...
    protected MediaFormat getMediaFormat(Format format, String str, int i, float f) {
        ...
        if (MimeTypes.AUDIO_AC4.equals(format.sampleMimeType) && (codecProfileAndLevel = CodecSpecificDataUtil.getCodecProfileAndLevel(format)) != null) {
        ...
        if (Objects.equals(format.sampleMimeType, MimeTypes.AUDIO_IAMF)) {
            ...
            if (audioCapabilities == null) {
                ...
                int iBitCount = Integer.bitCount(outputChannelMaskForCurrentConfiguration);
