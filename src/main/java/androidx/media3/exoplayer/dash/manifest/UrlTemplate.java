...
public final class UrlTemplate {
    ...
    private static void parseTemplate(String str, List<String> list, List<Integer> list2, List<String> list3) {
        ...
        while (length < str.length()) {
            int iIndexOf = str.indexOf(MetadataReservedKeys.PREFIX, length);
            if (iIndexOf == -1) {
                ...
                list.set(list2.size(), list.get(list2.size()) + MetadataReservedKeys.PREFIX);
                length += 2;
            } else {
                list3.add("");
                int i = length + 1;
                int iIndexOf2 = str.indexOf(MetadataReservedKeys.PREFIX, i);
