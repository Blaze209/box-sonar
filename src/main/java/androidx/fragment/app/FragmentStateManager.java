...
class FragmentStateManager {
    ...
    void resume() {
        ...
        if (focusedView != null && isFragmentViewChild(focusedView)) {
            ...
            if (FragmentManager.isLoggingEnabled(2)) {
                Log.v("FragmentManager", "requestFocus: Restoring focused view " + focusedView + " " + (zRequestFocus ? TelemetryEventStrings.Value.SUCCEEDED : TelemetryEventStrings.Value.FAILED) + " on Fragment " + this.mFragment + " resulting in focused view " + this.mFragment.mView.findFocus());
            }
        }
    ...
    Bundle saveState() {
        ...
        if (this.mFragment.mState > 0) {
            ...
            Bundle bundleM10245lambda$attachController$5$androidxfragmentappFragmentManager = this.mFragment.mChildFragmentManager.m10245lambda$attachController$5$androidxfragmentappFragmentManager();
            if (!bundleM10245lambda$attachController$5$androidxfragmentappFragmentManager.isEmpty()) {
                bundle.putBundle(CHILD_FRAGMENT_MANAGER_KEY, bundleM10245lambda$attachController$5$androidxfragmentappFragmentManager);
            }
            if (this.mFragment.mView != null) {
                saveViewState();
            }
