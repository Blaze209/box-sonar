...
final class AppCompatReceiveContentHelper {
    ...
    static boolean maybeHandleMenuActionViaPerformReceiveContent(TextView textView, int i) {
        ...
        ClipData primaryClip = clipboardManager == null ? null : MAMClipboard.getPrimaryClip(clipboardManager);
    ...
    private static final class OnDropApi24Impl {
        ...
        static boolean onDropForTextView(DragEvent dragEvent, TextView textView, Activity activity) {
            ...
            try {
                ...
                ViewCompat.performReceiveContent(textView, new ContentInfoCompat.Builder(MAMDragEventManagement.getClipData(dragEvent), 3).build());
                return true;
            } finally {
        ...
        static boolean onDropForView(DragEvent dragEvent, View view, Activity activity) {
            activity.requestDragAndDropPermissions(dragEvent);
            ViewCompat.performReceiveContent(view, new ContentInfoCompat.Builder(MAMDragEventManagement.getClipData(dragEvent), 3).build());
            return true;
        }
    }
