...
public final class HctSolver {
    ...
    private final double trueDelinearized(double rgbComponent) {
        ...
        return (d <= 0.0031308d ? d * 12.92d : (Math.pow(d, 0.4166666666666667d) * 1.055d) - 0.055d) * ((double) 255);
    }
    ...
    private final int findResultByJ(double hueRadians, double chroma, double y) {
        ...
        while (i < 5) {
            char c2 = c;
            double d = dPow;
            double d2 = dSqrt / 100.0d;
            int i2 = i;
            double d3 = dSqrt;
            double dPow2 = Math.pow(((chroma == 0.0d || dSqrt == 0.0d) ? 0.0d : chroma / Math.sqrt(d2)) * d, 1.1111111111111112d);
            double aw = (((double) frame.getAw()) * Math.pow(d2, (1.0d / ((double) frame.getC())) / ((double) frame.getZ()))) / ((double) frame.getNbb());
            double d4 = (((0.305d + aw) * 23.0d) * dPow2) / (((23.0d * dCos) + ((((double) 11) * dPow2) * dCos2)) + ((108.0d * dPow2) * dSin));
            double d5 = d4 * dCos2;
            double d6 = d4 * dSin;
            double d7 = 460.0d * aw;
            ...
            double dInverseChromaticAdaptation = inverseChromaticAdaptation(d8);
            double dInverseChromaticAdaptation2 = inverseChromaticAdaptation(d9);
            double dInverseChromaticAdaptation3 = inverseChromaticAdaptation(d10);
            double[][] dArr = LINRGB_FROM_SCALED_DISCOUNT;
            double[] dArr2 = dArr[0];
            double d11 = (dArr2[0] * dInverseChromaticAdaptation) + (dArr2[c2] * dInverseChromaticAdaptation2) + (dArr2[2] * dInverseChromaticAdaptation3);
            double[] dArr3 = dArr[c2];
            double d12 = (dArr3[0] * dInverseChromaticAdaptation) + (dArr3[c2] * dInverseChromaticAdaptation2) + (dArr3[2] * dInverseChromaticAdaptation3);
            double[] dArr4 = dArr[2];
            double d13 = (dInverseChromaticAdaptation * dArr4[0]) + (dInverseChromaticAdaptation2 * dArr4[c2]) + (dInverseChromaticAdaptation3 * dArr4[2]);
