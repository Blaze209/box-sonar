...
public final class Frame {
    ...
    public static final class Companion {
        ...
        public final Frame make(float[] whitepoint, float adaptingLuminance, float backgroundLstar, float surround, boolean discountingIlluminant) {
            ...
            float fPow = 0.725f / ((float) Math.pow(d2, 0.2f));
            double d3 = 0.42f;
            float[] fArr5 = {(float) Math.pow(((fArr4[0] * fCbrt) * f6) / 100.0f, d3), (float) Math.pow(((fArr4[1] * fCbrt) * f7) / 100.0f, d3), (float) Math.pow(((fArr4[2] * fCbrt) * f8) / 100.0f, d3)};
            float f13 = fArr5[0];
            ...
            return new Frame(fYFromLstar, ((fArr6[0] * 2.0f) + fArr6[1] + (fArr6[2] * 0.05f)) * fPow, fPow, fPow, fLerp, f9, fArr4, fCbrt, (float) Math.pow(fCbrt, 0.25f), fSqrt, null);
        }
    }
