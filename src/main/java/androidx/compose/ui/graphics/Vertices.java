...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m6868toArgb8_81llA(colors.get(i).m6824unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            long jM6579unboximpl = points.get(i / 2).m6579unboximpl();
            if (i % 2 == 0) {
                fIntBitsToFloat = Float.intBitsToFloat((int) (jM6579unboximpl >> 32));
            } else {
                fIntBitsToFloat = Float.intBitsToFloat((int) (jM6579unboximpl & 4294967295L));
            }
