...
public abstract class ShaderBrush extends Brush {
    ...
    public abstract Shader mo6783createShaderuvyYCjk(long size);

    public ShaderBrush() {
        super(null);
        this.createdSize = Size.INSTANCE.m6646getUnspecifiedNHjbRc();
    }
    ...
    public final void m7146setTransformQ8lPUPs(float[] fArr) {
        this.transform = fArr;
        TransformShader transformShader = this.internalTransformShader;
        if (transformShader != null) {
            transformShader.m7230transformQ8lPUPs(fArr);
        }
    }
    ...
    public final void mo6759applyToPq9zytI(long size, Paint p, float alpha) {
        TransformShader transformShaderObtainTransformShader = this.internalTransformShader;
        if (transformShaderObtainTransformShader == null || !Size.m6634equalsimpl0(this.createdSize, size)) {
            if (Size.m6640isEmptyimpl(size)) {
                ...
                if (fArr != null) {
                    transformShaderObtainTransformShader.m7230transformQ8lPUPs(fArr);
                }
                transformShaderObtainTransformShader.setShader(mo6783createShaderuvyYCjk(size));
                this.internalTransformShader = transformShaderObtainTransformShader;
                this.createdSize = size;
            }
        }
        if (!Color.m6815equalsimpl0(p.mo6684getColor0d7_KjU(), Color.INSTANCE.m6840getBlack0d7_KjU())) {
            p.mo6690setColor8_81llA(Color.INSTANCE.m6840getBlack0d7_KjU());
        }
