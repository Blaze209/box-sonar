...
public abstract class ShadowRenderer {
    ...
    private long shadowTintColor = Color.INSTANCE.m6850getUnspecified0d7_KjU();
    private long cornerRadius = CornerRadius.INSTANCE.m6541getZerokKHJgLs();
    private long generatedSize = Size.INSTANCE.m6646getUnspecifiedNHjbRc();
    ...
    protected abstract void mo7533buildShadow_SMYjrA(DrawScope drawScope, long j, long j2, Path path);

    /* JADX INFO: renamed from: onDrawShadow-MLmccfk */
    protected abstract void mo7534onDrawShadowMLmccfk(DrawScope drawScope, long j, long j2, Path path, float f, ColorFilter colorFilter, Brush brush, int i);
    ...
    public final void m7544drawShadowerFMhIw(DrawScope drawScope, ColorFilter colorFilter, long j, long j2, Brush brush, float f, int i) {
        updateParamsFromOutline(this.outline);
        if (colorFilter == null) {
            colorFilter = (brush != null || j2 == 16) ? null : m7543obtainTint8_81llA(j2);
        }
        ColorFilter colorFilter2 = colorFilter;
        long j3 = this.generatedSize;
        if (j3 == InlineClassHelperKt.UnspecifiedPackedFloats || !Size.m6634equalsimpl0(j3, j) || this.generatedLayoutDirection != drawScope.getLayoutDirection() || this.generatedDensity != drawScope.getDensity()) {
            mo7533buildShadow_SMYjrA(drawScope, j, this.cornerRadius, this.path);
            this.generatedSize = j;
            this.generatedLayoutDirection = drawScope.getLayoutDirection();
            this.generatedDensity = drawScope.getDensity();
        }
        mo7534onDrawShadowMLmccfk(drawScope, j, this.cornerRadius, this.path, f, colorFilter2, brush, i);
    }

    public void invalidateShadow() {
        this.generatedSize = Size.INSTANCE.m6646getUnspecifiedNHjbRc();
    ...
    private final ColorFilter m7543obtainTint8_81llA(long color) {
        ColorFilter colorFilter = this.shadowTint;
        if (colorFilter != null && Color.m6815equalsimpl0(this.shadowTintColor, color)) {
            return colorFilter;
        }
        ColorFilter colorFilterM6855tintxETnrds$default = ColorFilter.Companion.m6855tintxETnrds$default(ColorFilter.INSTANCE, color, 0, 2, null);
        this.shadowTintColor = color;
        this.shadowTint = colorFilterM6855tintxETnrds$default;
        return colorFilterM6855tintxETnrds$default;
    }

    private final void updateParamsFromOutline(Outline outline) {
        if (outline instanceof Outline.Generic) {
            this.path = ((Outline.Generic) outline).getPath();
            this.cornerRadius = CornerRadius.INSTANCE.m6541getZerokKHJgLs();
            return;
        }
        if (outline instanceof Outline.Rounded) {
            ...
            if (RoundRectKt.isSimple(rounded.getRoundRect())) {
                this.path = null;
                this.cornerRadius = rounded.getRoundRect().m6619getTopLeftCornerRadiuskKHJgLs();
                return;
            } else {
