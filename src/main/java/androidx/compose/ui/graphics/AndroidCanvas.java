...
public final class AndroidCanvas implements Canvas {
    ...
    public void mo6665concat58bKbWc(float[] matrix) {
        if (MatrixKt.m7085isIdentity58bKbWc(matrix)) {
            return;
        }
        android.graphics.Matrix matrix2 = new android.graphics.Matrix();
        AndroidMatrixConversions_androidKt.m6681setFromEL8BTi8(matrix2, matrix);
        this.internalCanvas.concat(matrix2);
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: clipRect-N_I0leg, reason: not valid java name */
    public void mo6664clipRectN_I0leg(float left, float top, float right, float bottom, int clipOp) {
        this.internalCanvas.clipRect(left, top, right, bottom, m6673toRegionOp7u2Bmg(clipOp));
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: clipPath-mtrdD-E, reason: not valid java name */
    public void mo6663clipPathmtrdDE(Path path, int clipOp) {
        android.graphics.Canvas canvas = this.internalCanvas;
        if (path instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) path).getInternalPath(), m6673toRegionOp7u2Bmg(clipOp));
            return;
        }
    ...
    public final Region.Op m6673toRegionOp7u2Bmg(int i) {
        return ClipOp.m6798equalsimpl0(i, ClipOp.INSTANCE.m6802getDifferencertfAjoo()) ? Region.Op.DIFFERENCE : Region.Op.INTERSECT;
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* JADX INFO: renamed from: drawLine-Wko1d7g, reason: not valid java name */
    public void mo6669drawLineWko1d7g(long p1, long p2, Paint paint) {
    ...
    public void mo6666drawCircle9KIMszo(long center, float radius, Paint paint) {
    ...
    public void mo6667drawImaged4ec7I(ImageBitmap image, long topLeftOffset, Paint paint) {
    ...
    public void mo6668drawImageRectHPBpro0(ImageBitmap image, long srcOffset, long srcSize, long dstOffset, long dstSize, Paint paint) {
        ...
        rect.left = IntOffset.m9815getXimpl(srcOffset);
        rect.top = IntOffset.m9816getYimpl(srcOffset);
        rect.right = IntOffset.m9815getXimpl(srcOffset) + ((int) (srcSize >> 32));
        rect.bottom = IntOffset.m9816getYimpl(srcOffset) + ((int) (srcSize & 4294967295L));
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        Intrinsics.checkNotNull(rect2);
        rect2.left = IntOffset.m9815getXimpl(dstOffset);
        rect2.top = IntOffset.m9816getYimpl(dstOffset);
        rect2.right = IntOffset.m9815getXimpl(dstOffset) + ((int) (dstSize >> 32));
        rect2.bottom = IntOffset.m9816getYimpl(dstOffset) + ((int) (dstSize & 4294967295L));
    ...
    public void mo6670drawPointsO7TthRY(int pointMode, List<Offset> points, Paint paint) {
        if (PointMode.m7131equalsimpl0(pointMode, PointMode.INSTANCE.m7135getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m7131equalsimpl0(pointMode, PointMode.INSTANCE.m7137getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (PointMode.m7131equalsimpl0(pointMode, PointMode.INSTANCE.m7136getPointsr_lszbg())) {
    ...
    private final void drawLines(List<Offset> points, Paint paint, int stepBy) {
        if (points.size() >= 2) {
            ...
            while (i < points.size() - 1) {
                long jM6579unboximpl = points.get(i).m6579unboximpl();
                long jM6579unboximpl2 = points.get(i + 1).m6579unboximpl();
                this.internalCanvas.drawLine(Float.intBitsToFloat((int) (jM6579unboximpl >> 32)), Float.intBitsToFloat((int) (jM6579unboximpl & 4294967295L)), Float.intBitsToFloat((int) (jM6579unboximpl2 >> 32)), Float.intBitsToFloat((int) (jM6579unboximpl2 & 4294967295L)), internalPaint);
                i += stepBy;
            }
        }
    }
    ...
    public void mo6671drawRawPointsO7TthRY(int pointMode, float[] points, Paint paint) {
    ...
    public void mo6672drawVerticesTPEHhCM(Vertices vertices, int blendMode, Paint paint) {
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m6718toAndroidVertexModeJOOmi9M(vertices.getVertexMode()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.getInternalPaint());
    }
    ...
    private final void drawPoints(List<Offset> points, Paint paint) {
        ...
        for (int i = 0; i < size; i++) {
            long jM6579unboximpl = points.get(i).m6579unboximpl();
            this.internalCanvas.drawPoint(Float.intBitsToFloat((int) (jM6579unboximpl >> 32)), Float.intBitsToFloat((int) (jM6579unboximpl & 4294967295L)), paint.getInternalPaint());
        }
    }
}
