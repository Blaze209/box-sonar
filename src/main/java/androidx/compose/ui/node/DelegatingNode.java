...
public abstract class DelegatingNode extends Modifier.Node {
    ...
    protected final <T extends DelegatableNode> T delegate(T delegatableNode) {
        ...
        if (getIsAttached()) {
            if ((iCalculateNodeKindSetFromIncludingDelegates & NodeKind.m8585constructorimpl(2)) != 0 && (kindSet$ui & NodeKind.m8585constructorimpl(2)) == 0) {
    ...
    protected final void undelegate(DelegatableNode instance) {
        ...
        for (Modifier.Node child = this.delegate; child != null; child = child.getChild()) {
            if (child == instance) {
                ...
                if (getIsAttached() && (kindSet$ui & NodeKind.m8585constructorimpl(2)) != 0 && (NodeKind.m8585constructorimpl(2) & iCalculateNodeKindSetFromIncludingDelegates) == 0) {
                    NodeChain nodes = DelegatableNodeKt.requireLayoutNode(this).getNodes();
                    getNode().updateCoordinator$ui(null);
                    nodes.syncCoordinators();
                    return;
    ...
    private final void validateDelegateKindSet(int delegateKindSet, Modifier.Node delegateNode) {
        int kindSet$ui = getKindSet();
        if ((delegateKindSet & NodeKind.m8585constructorimpl(2)) == 0 || (NodeKind.m8585constructorimpl(2) & kindSet$ui) == 0 || (this instanceof LayoutModifierNode)) {
            return;
        }
