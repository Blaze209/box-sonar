...
public final class AndroidParagraphHelper_androidKt {
    ...
    public static final CharSequence createCharSequence(String str, float f, TextStyle textStyle, List<? extends AnnotatedString.Range<? extends AnnotatedString.Annotation>> list, List<AnnotatedString.Range<Placeholder>> list2, Density density, Function4<? super FontFamily, ? super FontWeight, ? super FontStyle, ? super FontSynthesis, ? extends Typeface> function4, boolean z) {
        ...
        if (z && EmojiCompat.isConfigured()) {
            ...
            strProcess = EmojiCompat.get().process(str, 0, str.length(), Integer.MAX_VALUE, emojiSupportMatchM8911boximpl == null ? 0 : EmojiSupportMatch.m8914equalsimpl0(emojiSupportMatchM8911boximpl.getValue(), EmojiSupportMatch.INSTANCE.m8918getAll_3YsG6Y()));
            Intrinsics.checkNotNull(strProcess);
        } else {
            strProcess = str;
        }
        if (list.isEmpty() && list2.isEmpty() && Intrinsics.areEqual(textStyle.getTextIndent(), TextIndent.INSTANCE.getNone()) && TextUnit.m9879getRawTypeimpl(textStyle.m9130getLineHeightXSAIIZE()) == 0) {
            return strProcess;
        }
        ...
        if (isIncludeFontPaddingEnabled(textStyle) && textStyle.getLineHeightStyle() == null) {
            SpannableExtensions_androidKt.m9379setLineHeightr9BaKPg(spannable, textStyle.m9130getLineHeightXSAIIZE(), f, density);
            f2 = f;
            density2 = density;
        } else {
            ...
            f2 = f;
            density2 = density;
            SpannableExtensions_androidKt.m9378setLineHeightKmRG4DE(spannable, textStyle.m9130getLineHeightXSAIIZE(), f2, density2, lineHeightStyle);
        }
