...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m9089getMinimpl(j);
        this.selectionEnd = TextRange.m9088getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int iM9089getMinimpl = TextRange.m9089getMinimpl(j);
        int iM9088getMaximpl = TextRange.m9088getMaximpl(j);
        if (iM9089getMinimpl < 0 || iM9089getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + iM9089getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM9088getMaximpl < 0 || iM9088getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + iM9088getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM9089getMinimpl > iM9088getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + iM9089getMinimpl + " > " + iM9088getMaximpl);
        }
    }
    ...
    public final TextRange m9257getCompositionMzsxiRA$ui_text() {
        if (hasComposition$ui_text()) {
            return TextRange.m9079boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* JADX INFO: renamed from: getSelection-d9O1mEE$ui_text, reason: not valid java name */
    public final long m9258getSelectiond9O1mEE$ui_text() {
    ...
    public final void delete$ui_text(int start, int end) {
        ...
        long jM9259updateRangeAfterDeletepWDy79M = EditingBufferKt.m9259updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m9089getMinimpl(jM9259updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m9088getMaximpl(jM9259updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text()) {
            long jM9259updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m9259updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m9085getCollapsedimpl(jM9259updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text();
            } else {
                this.compositionStart = TextRange.m9089getMinimpl(jM9259updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m9088getMaximpl(jM9259updateRangeAfterDeletepWDy79M2);
            }
        }
    }
