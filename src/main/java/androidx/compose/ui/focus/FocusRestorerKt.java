                            ...
                            if (nodePop instanceof FocusTargetNode) {
                                ...
                                if (focusTargetNode3.getIsAttached() && DelegatableNodeKt.requireLayoutNode(focusTargetNode3).getCompositeKeyHash() == focusTargetNode.getPreviouslyFocusedChildHash()) {
                                    if (ComposeUiFlags.isRequestFocusOnNonFocusableFocusTargetEnabled) {
                                        return restoreFocusedChild(focusTargetNode3) || (focusTargetNode3.fetchFocusProperties$ui().getCanFocus() && FocusTargetModifierNode.m6476requestFocus3ESFkO8$default(focusTargetNode3, 0, 1, null));
                                    }
                                    return restoreFocusedChild(focusTargetNode3) || FocusTargetModifierNode.m6476requestFocus3ESFkO8$default(focusTargetNode3, 0, 1, null);
                                }
                            } else if ((nodePop.getKindSet() & iM8585constructorimpl) != 0 && (nodePop instanceof DelegatingNode)) {
                                int i = 0;
                                for (Modifier.Node delegate = ((DelegatingNode) nodePop).getDelegate(); delegate != null; delegate = delegate.getChild()) {
