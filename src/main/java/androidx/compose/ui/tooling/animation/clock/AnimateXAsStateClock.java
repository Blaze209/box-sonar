...
public final class AnimateXAsStateClock<T, V extends AnimationVector> implements ComposeAnimationClock<AnimateXAsStateComposeAnimation<T, V>, TargetState<T>> {
    ...
    private TargetState<T> state = new TargetState<>(getAnimation().m9599getAnimationObject().getValue(), getAnimation().m9599getAnimationObject().getValue());
    ...
    private final TargetBasedAnimation<T, V> getCurrentAnimation() {
        return AnimationKt.TargetBasedAnimation(getAnimation().getAnimationSpec(), getAnimation().m9599getAnimationObject().getTypeConverter(), getState().getInitial(), getState().getTarget(), getAnimation().m9599getAnimationObject().getVelocity());
    }
}
