...
final class AnimatedEnterExitMeasurePolicy implements MeasurePolicy {
    ...
    public MeasureResult mo344measure3p2s80s(MeasureScope measureScope, List<? extends Measurable> list, long j) {
        ...
        for (int i = 0; i < size; i++) {
            Placeable placeableMo8265measureBRTryo0 = list.get(i).mo8265measureBRTryo0(j);
            iMax = Math.max(iMax, placeableMo8265measureBRTryo0.getWidth());
            iMax2 = Math.max(iMax2, placeableMo8265measureBRTryo0.getHeight());
            arrayList.add(placeableMo8265measureBRTryo0);
        }
        final ArrayList arrayList2 = arrayList;
        if (measureScope.isLookingAhead()) {
            this.hasLookaheadOccurred = true;
            this.scope.getTargetSize$animation().setValue(IntSize.m9850boximpl(IntSize.m9853constructorimpl((((long) iMax2) & 4294967295L) | (((long) iMax) << 32))));
