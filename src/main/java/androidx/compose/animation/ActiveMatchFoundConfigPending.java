...
public final class ActiveMatchFoundConfigPending extends SharedTransitionStateMachine.State {
    ...
    public SharedTransitionStateMachine.State mo339configureActiveMatch38uP1EE(SharedElement sharedElement, BoundsProvider targetBoundsProvider, long lookaheadSize, long topLeft, long structuralOffset) {
        ...
        if (targetData == null) {
            targetData = new TargetData(lookaheadSize, Offset.m6573minusMKHz9U(topLeft, structuralOffset), structuralOffset, null);
        }
        Rect currentBounds = getCurrentBounds();
        if (currentBounds != null) {
            j = topLeft;
            ...
            if (currentBounds == null) {
                j = topLeft;
                currentBounds = RectKt.m6609Recttz77jQw(j, lookaheadSize);
            } else {
                j = topLeft;
            }
        }
        TargetData targetData2 = targetData;
        SharedTransitionStateMachineKt.m434updateTargetDataBGTQxF0(targetData2, lookaheadSize, j, structuralOffset, true);
