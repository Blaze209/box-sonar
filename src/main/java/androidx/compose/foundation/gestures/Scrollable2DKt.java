...
public final class Scrollable2DKt {
    ...
    public static final Object m939semanticsScrollByd4ec7I(ScrollingLogic2D scrollingLogic2D, long j, Continuation<? super Offset> continuation) {
    ...
    public static final float m938getMagnitudeTH1AsA0(long j) {
        double d = 2;
        return (float) Math.sqrt(((float) Math.pow(Velocity.m9925getXimpl(j), d)) + ((float) Math.pow(Velocity.m9926getYimpl(j), d)));
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX INFO: renamed from: getAngle-TH1AsA0, reason: not valid java name */
    public static final float m937getAngleTH1AsA0(long j) {
        return (float) Math.atan2(Velocity.m9926getYimpl(j), Velocity.m9925getXimpl(j));
    }
}
