...
public final class TextFieldPreparedSelection extends BaseTextPreparedSelection<TextFieldPreparedSelection> {
    ...
    public final TextFieldValue getValue() {
        return TextFieldValue.m9340copy3r_uNRQ$default(this.currentValue, getAnnotatedString(), getSelection(), (TextRange) null, 4, (Object) null);
    }

    public final List<EditCommand> deleteIfSelectedOr(Function1<? super TextFieldPreparedSelection, ? extends EditCommand> or) {
        if (!TextRange.m9085getCollapsedimpl(getSelection())) {
            return CollectionsKt.listOf((Object[]) new EditCommand[]{new CommitTextCommand("", 0), new SetSelectionCommand(TextRange.m9089getMinimpl(getSelection()), TextRange.m9089getMinimpl(getSelection()))});
        }
    ...
    private final int jumpByPagesOffset(TextLayoutResultProxy textLayoutResultProxy, int i) {
        ...
        Rect cursorRect = textLayoutResultProxy.getValue().getCursorRect(getOffsetMapping().originalToTransformed(TextRange.m9086getEndimpl(this.currentValue.getSelection())));
        return getOffsetMapping().transformedToOriginal(textLayoutResultProxy.getValue().m9061getOffsetForPositionk4lQ0M(Offset.m6561constructorimpl((((long) Float.floatToRawIntBits(cursorRect.getLeft())) << 32) | (((long) Float.floatToRawIntBits(cursorRect.getTop() + (Float.intBitsToFloat((int) (zero.m6602getSizeNHjbRc() & 4294967295L)) * i))) & 4294967295L))));
    }
}
