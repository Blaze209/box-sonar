...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m9087getLengthimpl(textFieldBuffer.getChanges().mo1824getRangejx7JFs(0)) != 1 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int iM9089getMinimpl = TextRange.m9089getMinimpl(textFieldBuffer.getChanges().mo1824getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation() != iM9089getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(iM9089getMinimpl);
        }
    }
