...
final class LazyGridCacheWindowScope implements CacheWindowScope {
    ...
    public int getLastIndexInLine(int lineIndex) {
        ...
        List<Pair<Integer, Constraints>> listInvoke = lazyGridMeasureResult.getPrefetchInfoRetriever().invoke(Integer.valueOf(lineIndex));
        if (listInvoke.isEmpty()) {
            return -1;
        }
        return (((Number) ((Pair) CollectionsKt.first((List) listInvoke)).getFirst()).intValue() + listInvoke.size()) - 1;
    }
