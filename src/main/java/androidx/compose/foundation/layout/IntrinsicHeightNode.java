...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo1166calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        int iMaxIntrinsicHeight;
        if (this.height == IntrinsicSize.Min) {
            iMaxIntrinsicHeight = measurable.minIntrinsicHeight(Constraints.m9640getMaxWidthimpl(j));
        } else {
            iMaxIntrinsicHeight = measurable.maxIntrinsicHeight(Constraints.m9640getMaxWidthimpl(j));
        }
        if (iMaxIntrinsicHeight < 0) {
            iMaxIntrinsicHeight = 0;
        }
        return Constraints.INSTANCE.m9651fixedHeightOenEA2s(iMaxIntrinsicHeight);
    }
