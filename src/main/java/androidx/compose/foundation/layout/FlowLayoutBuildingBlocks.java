...
public final class FlowLayoutBuildingBlocks {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        wrapEllipsisInfoEllipsisInfo$foundation_layout.setPlaceEllipsisOnLastContentLine(lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m331getFirstimpl(wrapEllipsisInfoEllipsisInfo$foundation_layout.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis)));
    ...
    public final WrapInfo m1126getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        int i = totalCrossAxisSize + currentLineCrossAxisSize;
        if (nextSize == null) {
            return new WrapInfo(true, true);
        }
        if (this.overflow.getType$foundation_layout() != FlowLayoutOverflow.OverflowType.Visible && (lineIndex >= this.maxLines || IntIntPair.m332getSecondimpl(leftOver) - IntIntPair.m332getSecondimpl(nextSize.getPackedValue()) < 0)) {
            return new WrapInfo(true, true);
        }
        if (nextIndexInLine != 0 && (nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m331getFirstimpl(leftOver) - IntIntPair.m331getFirstimpl(nextSize.getPackedValue()) < 0)) {
            if (isWrappingRound) {
                return new WrapInfo(true, true);
            }
            return new WrapInfo(true, m1126getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m327constructorimpl(Constraints.m9640getMaxWidthimpl(this.constraints), (IntIntPair.m332getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m324boximpl(IntIntPair.m327constructorimpl(IntIntPair.m331getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m332getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, i, 0, true, false).getIsLastItemInContainer());
        }
        int iMax = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m332getSecondimpl(nextSize.getPackedValue()));
        IntIntPair intIntPairM1135ellipsisSizeF35zmw$foundation_layout = isEllipsisWrap ? null : this.overflow.m1135ellipsisSizeF35zmw$foundation_layout(nextItemHasNext, lineIndex, iMax);
        if (intIntPairM1135ellipsisSizeF35zmw$foundation_layout != null) {
            intIntPairM1135ellipsisSizeF35zmw$foundation_layout.getPackedValue();
            if (nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m331getFirstimpl(leftOver) - IntIntPair.m331getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m331getFirstimpl(intIntPairM1135ellipsisSizeF35zmw$foundation_layout.getPackedValue()) < 0) {
                if (isEllipsisWrap) {
                    return new WrapInfo(true, true);
                }
                WrapInfo wrapInfoM1126getWrapInfoOpUlnko = m1126getWrapInfoOpUlnko(false, 0, IntIntPair.m327constructorimpl(Constraints.m9640getMaxWidthimpl(this.constraints), (IntIntPair.m332getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m332getSecondimpl(nextSize.getPackedValue()))), intIntPairM1135ellipsisSizeF35zmw$foundation_layout, lineIndex + 1, iMax, 0, true, true);
                return new WrapInfo(wrapInfoM1126getWrapInfoOpUlnko.getIsLastItemInContainer(), wrapInfoM1126getWrapInfoOpUlnko.getIsLastItemInContainer());
            }
        }
