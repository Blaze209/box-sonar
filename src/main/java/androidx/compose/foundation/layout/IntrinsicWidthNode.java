...
final class IntrinsicWidthNode extends IntrinsicSizeModifier {
    ...
    public long mo1166calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        if (this.width == IntrinsicSize.Min) {
            iMaxIntrinsicWidth = measurable.minIntrinsicWidth(Constraints.m9639getMaxHeightimpl(j));
        } else {
            iMaxIntrinsicWidth = measurable.maxIntrinsicWidth(Constraints.m9639getMaxHeightimpl(j));
        }
        if (iMaxIntrinsicWidth < 0) {
            iMaxIntrinsicWidth = 0;
        }
        return Constraints.INSTANCE.m9652fixedWidthOenEA2s(iMaxIntrinsicWidth);
    }
