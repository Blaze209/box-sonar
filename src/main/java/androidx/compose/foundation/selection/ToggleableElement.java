...
final class ToggleableElement extends ModifierNodeElement<ToggleableNode> {
    ...
    public void update(ToggleableNode node) {
        node.m1553updateO2vRcR0(this.value, this.interactionSource, this.indicationNodeFactory, this.useLocalIndication, this.enabled, this.role, this.onValueChange);
    }
    ...
    public int hashCode() {
        ...
        Role role = this.role;
        return ((iHashCode3 + (role != null ? Role.m8829hashCodeimpl(role.getValue()) : 0)) * 31) + this.onValueChange.hashCode();
    }
}
