...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver, RippleHostKey {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> state, State<RippleAlpha> state2, ViewGroup viewGroup) {
        ...
        this.rippleSize = Size.INSTANCE.m6647getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int iRoundToInt;
        this.rippleSize = contentDrawScope.mo7395getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            iRoundToInt = MathKt.roundToInt(RippleAnimationKt.m2700getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo7395getSizeNHjbRc()));
        } else {
            iRoundToInt = contentDrawScope.mo748roundToPx0680j_4(this.radius);
        }
        ...
        if (rippleHostView != null) {
            rippleHostView.m2704setRipplePropertiesbiQXAtU(contentDrawScope.mo7395getSizeNHjbRc(), this.rippleRadius, jM6824unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m2703addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m6824unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
