...
public abstract class Ripple implements Indication {
    ...
    public abstract RippleIndicationInstance mo2696rememberUpdatedRippleInstance942rkJo(InteractionSource interactionSource, boolean z, float f, State<Color> state, State<RippleAlpha> state2, Composer composer, int i);
    ...
    public final IndicationInstance rememberUpdatedInstance(InteractionSource interactionSource, Composer composer, int i) {
        long jMo2698defaultColorWaAFU9c;
        ...
        if (this.color.getValue().m6824unboximpl() != 16) {
            composer.startReplaceGroup(762841340);
            composer.endReplaceGroup();
            jMo2698defaultColorWaAFU9c = this.color.getValue().m6824unboximpl();
        } else {
            composer.startReplaceGroup(762899124);
            ComposerKt.sourceInformation(composer, "197@9740L14");
            jMo2698defaultColorWaAFU9c = rippleTheme.mo2698defaultColorWaAFU9c(composer, 0);
            composer.endReplaceGroup();
        }
        State<Color> stateRememberUpdatedState = SnapshotStateKt.rememberUpdatedState(Color.m6804boximpl(jMo2698defaultColorWaAFU9c), composer, 0);
        ...
        int i2 = i & 14;
        RippleIndicationInstance rippleIndicationInstanceMo2696rememberUpdatedRippleInstance942rkJo = mo2696rememberUpdatedRippleInstance942rkJo(interactionSource, this.bounded, this.radius, stateRememberUpdatedState, stateRememberUpdatedState2, composer, i2 | ((i << 12) & 458752));
        ComposerKt.sourceInformationMarkerStart(composer, 994453534, "CC(remember):Ripple.kt#9igjgp");
        boolean zChangedInstance = composer.changedInstance(rippleIndicationInstanceMo2696rememberUpdatedRippleInstance942rkJo) | (((i2 ^ 6) > 4 && composer.changed(interactionSource)) || (i & 6) == 4);
        ...
        if (zChangedInstance || ripple$rememberUpdatedInstance$1$1RememberedValue == Composer.INSTANCE.getEmpty()) {
            ripple$rememberUpdatedInstance$1$1RememberedValue = new Ripple$rememberUpdatedInstance$1$1(interactionSource, rippleIndicationInstanceMo2696rememberUpdatedRippleInstance942rkJo, null);
            composer.updateRememberedValue(ripple$rememberUpdatedInstance$1$1RememberedValue);
        }
        ComposerKt.sourceInformationMarkerEnd(composer);
        EffectsKt.LaunchedEffect(rippleIndicationInstanceMo2696rememberUpdatedRippleInstance942rkJo, interactionSource, (Function2) ripple$rememberUpdatedInstance$1$1RememberedValue, composer, (i << 3) & 112);
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventEnd();
        }
        composer.endReplaceGroup();
        return rippleIndicationInstanceMo2696rememberUpdatedRippleInstance942rkJo;
    }
    ...
    public boolean equals(Object other) {
        ...
        return this.bounded == ripple.bounded && Dp.m9692equalsimpl0(this.radius, ripple.radius) && Intrinsics.areEqual(this.color, ripple.color);
    }

    public int hashCode() {
        return (((Boolean.hashCode(this.bounded) * 31) + Dp.m9693hashCodeimpl(this.radius)) * 31) + this.color.hashCode();
    }
}
