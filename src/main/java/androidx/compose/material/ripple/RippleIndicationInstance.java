...
public abstract class RippleIndicationInstance implements IndicationInstance {
    ...
    public final void m2706drawStateLayerH2RKhps(DrawScope drawScope, float f, long j) {
        float fM2700getRippleEndRadiuscSwnlzA;
        StateLayer stateLayer = this.stateLayer;
        if (Float.isNaN(f)) {
            fM2700getRippleEndRadiuscSwnlzA = RippleAnimationKt.m2700getRippleEndRadiuscSwnlzA(drawScope, this.bounded, drawScope.mo7395getSizeNHjbRc());
        } else {
            fM2700getRippleEndRadiuscSwnlzA = drawScope.mo754toPx0680j_4(f);
        }
        stateLayer.m2714drawStateLayermxwnekA(drawScope, fM2700getRippleEndRadiuscSwnlzA, j);
    }
}
