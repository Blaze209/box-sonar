...
class MediaSessionManagerImplBase implements MediaSessionManager.MediaSessionManagerImpl {
    ...
    public boolean isTrustedForMediaControl(MediaSessionManager.RemoteUserInfoImpl remoteUserInfoImpl) {
        try {
            if (MAMPackageManagement.getApplicationInfo(this.mContext.getPackageManager(), remoteUserInfoImpl.getPackageName(), 0) == null) {
                return false;
            }
    ...
    private boolean isPermissionGranted(MediaSessionManager.RemoteUserInfoImpl remoteUserInfoImpl, String str) {
        if (remoteUserInfoImpl.getPid() < 0) {
            return MAMPackageManagement.checkPermission(this.mContext.getPackageManager(), str, remoteUserInfoImpl.getPackageName()) == 0;
        }
