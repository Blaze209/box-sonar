...
public class SharedPrefManager {
    ...
    public synchronized void clearIncompatibleModelInfo(RemoteModel remoteModel) {
        zza().edit().remove(String.format("bad_hash_%s", remoteModel.getUniqueModelNameForPersist())).remove(SplitConfiguration.SPLIT_ATTRIBUTE_APP_VERSION_KEY).apply();
    }
    ...
    public synchronized String getPreviousAppVersion() {
        return zza().getString(SplitConfiguration.SPLIT_ATTRIBUTE_APP_VERSION_KEY, null);
    }
    ...
    public synchronized void setIncompatibleModelInfo(RemoteModel remoteModel, String str, String str2) {
        zza().edit().putString(String.format("bad_hash_%s", remoteModel.getUniqueModelNameForPersist()), str).putString(SplitConfiguration.SPLIT_ATTRIBUTE_APP_VERSION_KEY, str2).apply();
    }
