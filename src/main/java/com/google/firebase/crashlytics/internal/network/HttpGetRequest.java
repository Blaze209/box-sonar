...
public class HttpGetRequest {
    ...
    private String createUrlWithParams(String str, Map<String, String> map) throws UnsupportedEncodingException {
        ...
        if (!str.contains(MsalUtils.QUERY_STRING_SYMBOL)) {
            return str + MsalUtils.QUERY_STRING_SYMBOL + strCreateParamsString;
        }
        if (!str.endsWith(MsalUtils.QUERY_STRING_DELIMITER)) {
            strCreateParamsString = MsalUtils.QUERY_STRING_DELIMITER + strCreateParamsString;
        }
    ...
    private String createParamsString(Map<String, String> map) throws UnsupportedEncodingException {
        ...
        sb.append(next.getKey()).append(SimpleComparison.EQUAL_TO_OPERATION).append(next.getValue() != null ? URLEncoder.encode(next.getValue(), "UTF-8") : "");
        while (it.hasNext()) {
            Map.Entry<String, String> next2 = it.next();
            sb.append(MsalUtils.QUERY_STRING_DELIMITER).append(next2.getKey()).append(SimpleComparison.EQUAL_TO_OPERATION).append(next2.getValue() != null ? URLEncoder.encode(next2.getValue(), "UTF-8") : "");
        }
        return sb.toString();
    }
