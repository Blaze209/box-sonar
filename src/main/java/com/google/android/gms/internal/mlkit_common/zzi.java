...
public final class zzi {
    ...
    public static AssetFileDescriptor zza(Context context, Uri uri, String str) throws FileNotFoundException {
        ...
        if (UriUtil.QUALIFIED_RESOURCE_SCHEME.equals(scheme)) {
            return MAMContentResolverManagement.openAssetFileDescriptor(contentResolver, uriZzc, "r");
        }
        if ("content".equals(scheme)) {
            if (!zzj(context, uriZzc, 1, zzhVar)) {
                throw new FileNotFoundException("Can't open content uri.");
            }
            AssetFileDescriptor assetFileDescriptorOpenAssetFileDescriptor = MAMContentResolverManagement.openAssetFileDescriptor(contentResolver, uriZzc, "r");
        ...
        AssetFileDescriptor assetFileDescriptorOpenAssetFileDescriptor2 = MAMContentResolverManagement.openAssetFileDescriptor(contentResolver, uriZzc, "r");
    ...
    public static InputStream zzb(Context context, Uri uri, zzh zzhVar) throws FileNotFoundException {
        ...
        String scheme = uriZzc.getScheme();
        if (UriUtil.QUALIFIED_RESOURCE_SCHEME.equals(scheme)) {
            return MAMContentResolverManagement.openInputStream(contentResolver, uriZzc);
        }
        if ("content".equals(scheme)) {
            if (!zzj(context, uriZzc, 1, zzhVar)) {
                throw new FileNotFoundException("Can't open content uri.");
            }
            InputStream inputStreamOpenInputStream = MAMContentResolverManagement.openInputStream(contentResolver, uriZzc);
        ...
        try {
            ParcelFileDescriptor parcelFileDescriptorOpenFileDescriptor = MAMContentResolverManagement.openFileDescriptor(contentResolver, Uri.fromFile(new File(uriZzc.getPath()).getCanonicalFile()), "r");
            try {
    ...
    private static boolean zzj(Context context, Uri uri, int i, zzh zzhVar) {
        ...
        ProviderInfo providerInfoResolveContentProvider = MAMPackageManagement.resolveContentProvider(context.getPackageManager(), authority, 0);
        if (providerInfoResolveContentProvider == null) {
            int iLastIndexOf = authority.lastIndexOf(64);
            if (iLastIndexOf >= 0) {
                authority = authority.substring(iLastIndexOf + 1);
                providerInfoResolveContentProvider = MAMPackageManagement.resolveContentProvider(context.getPackageManager(), authority, 0);
            }
            if (providerInfoResolveContentProvider == null) {
                return !zzhVar.zzb;
            }
