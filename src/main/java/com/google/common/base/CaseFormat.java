...
public abstract class CaseFormat {
    ...
    public static final CaseFormat LOWER_HYPHEN = new CaseFormat("LOWER_HYPHEN", 0, CharMatcher.is(Soundex.SILENT_MARKER), CacheKeyValueDelegate.CACHE_VALUE_SEPARATOR) { // from class: com.google.common.base.CaseFormat.1
        ...
        String convert(CaseFormat format, String s) {
            if (format == LOWER_UNDERSCORE) {
                return s.replace(Soundex.SILENT_MARKER, SessionDataKt.UNDERSCORE);
            }
            if (format == UPPER_UNDERSCORE) {
                return Ascii.toUpperCase(s.replace(Soundex.SILENT_MARKER, SessionDataKt.UNDERSCORE));
            }
    ...
    static {
        String str = "_";
        LOWER_UNDERSCORE = new CaseFormat("LOWER_UNDERSCORE", 1, CharMatcher.is(SessionDataKt.UNDERSCORE), str) { // from class: com.google.common.base.CaseFormat.2
            @Override // com.google.common.base.CaseFormat
            String normalizeWord(String word) {
                return Ascii.toLowerCase(word);
            }
            ...
            String convert(CaseFormat format, String s) {
                if (format == LOWER_HYPHEN) {
                    return s.replace(SessionDataKt.UNDERSCORE, Soundex.SILENT_MARKER);
                }
                if (format == UPPER_UNDERSCORE) {
        ...
        UPPER_UNDERSCORE = new CaseFormat("UPPER_UNDERSCORE", 4, CharMatcher.is(SessionDataKt.UNDERSCORE), str) { // from class: com.google.common.base.CaseFormat.5
            @Override // com.google.common.base.CaseFormat
            String normalizeWord(String word) {
            ...
            String convert(CaseFormat format, String s) {
                if (format == LOWER_HYPHEN) {
                    return Ascii.toLowerCase(s.replace(SessionDataKt.UNDERSCORE, Soundex.SILENT_MARKER));
                }
                if (format == LOWER_UNDERSCORE) {
