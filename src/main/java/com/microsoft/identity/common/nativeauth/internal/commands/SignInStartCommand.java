...
public final class SignInStartCommand extends BaseNativeAuthCommand<SignInStartCommandResult> {
    ...
    public SignInStartCommandResult execute() throws Exception {
        LogSession.Companion companion = LogSession.INSTANCE;
        String TAG2 = TAG;
        Intrinsics.checkNotNullExpressionValue(TAG2, "TAG");
        companion.logMethodCall(TAG2, this.parameters.getCorrelationId(), TAG2 + ".execute");
        SignInStartCommandResult signInStartCommandResultSignInStart = this.controller.signInStart(this.parameters);
        Logger.infoWithObject(TAG2, this.parameters.getCorrelationId(), "Returning result: ", signInStartCommandResultSignInStart);
        return signInStartCommandResultSignInStart;
    }
}
