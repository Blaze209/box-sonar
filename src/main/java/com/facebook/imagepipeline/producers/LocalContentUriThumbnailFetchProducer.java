...
public class LocalContentUriThumbnailFetchProducer extends LocalFetchProducer implements ThumbnailProducer<EncodedImage> {
    ...
    private EncodedImage getCameraImage(Uri uri, @Nullable ResizeOptions resizeOptions) throws IOException {
        ...
        if (resizeOptions == null || (cursorQuery = MAMContentResolverManagement.query(this.mContentResolver, uri, PROJECTION, null, null, null)) == null) {
            return null;
        }
        try {
            if (!cursorQuery.moveToFirst() || (thumbnail = getThumbnail(resizeOptions, cursorQuery.getLong(cursorQuery.getColumnIndex(FieldType.FOREIGN_ID_FIELD_SUFFIX)))) == null) {
                return null;
            }
