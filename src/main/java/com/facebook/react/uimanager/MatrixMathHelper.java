...
public final class MatrixMathHelper {
    ...
    public static final void decomposeMatrix(double[] transformMatrix, MatrixDecompositionContext ctx) {
        ...
        if (matrixMathHelper.isZero(dArr7[0][3])) {
            ...
            double dV3Dot = v3Dot(dArrV3Normalize2, dArr[c]);
            dArr4[0] = dV3Dot;
            double[] dArrV3Combine = v3Combine(dArr[c], dArr[0], 1.0d, -dV3Dot);
            dArr[c] = dArrV3Combine;
            double dV3Length2 = v3Length(dArrV3Combine);
            dArr3[c] = dV3Length2;
            dArr[c] = v3Normalize(dArr[c], dV3Length2);
            dArr4[0] = dArr4[0] / dArr3[c];
            double dV3Dot2 = v3Dot(dArr[0], dArr[2]);
            dArr4[c] = dV3Dot2;
            double[] dArrV3Combine2 = v3Combine(dArr[2], dArr[0], 1.0d, -dV3Dot2);
            dArr[2] = dArrV3Combine2;
            double dV3Dot3 = v3Dot(dArr[c], dArrV3Combine2);
            dArr4[2] = dV3Dot3;
            double[] dArrV3Combine3 = v3Combine(dArr[2], dArr[c], 1.0d, -dV3Dot3);
            dArr[2] = dArrV3Combine3;
            double dV3Length3 = v3Length(dArrV3Combine3);
            dArr3[2] = dV3Length3;
            dArrV3Normalize = v3Normalize(dArr[2], dV3Length3);
            dArr[2] = dArrV3Normalize;
            double d2 = dArr4[c];
            double d3 = dArr3[2];
            dArr4[c] = d2 / d3;
            dArr4[2] = dArr4[2] / d3;
            if (v3Dot(dArr[0], v3Cross(dArr[c], dArrV3Normalize)) < 0.0d) {
                for (i4 = 0; i4 < 3; i4++) {
            ...
            dArr6[0] = roundTo3Places((-Math.atan2(dArr12[c], dArr12[2])) * 57.29577951308232d);
            double[] dArr13 = dArr[2];
            double d4 = -dArr13[0];
            double d5 = dArr13[c];
            double d6 = dArr13[2];
            dArr6[c] = roundTo3Places((-Math.atan2(d4, Math.sqrt((d5 * d5) + (d6 * d6)))) * 57.29577951308232d);
            dArr6[2] = roundTo3Places((-Math.atan2(dArr[c][0], dArr[0][0])) * 57.29577951308232d);
        }
        c = 1;
        double d7 = dArr7[0][3];
        double d8 = dArr7[c][3];
        ...
        double dV3Dot4 = v3Dot(dArrV3Normalize3, dArr[c]);
        dArr4[0] = dV3Dot4;
        double[] dArrV3Combine4 = v3Combine(dArr[c], dArr[0], 1.0d, -dV3Dot4);
        dArr[c] = dArrV3Combine4;
        double dV3Length5 = v3Length(dArrV3Combine4);
        dArr3[c] = dV3Length5;
        dArr[c] = v3Normalize(dArr[c], dV3Length5);
        dArr4[0] = dArr4[0] / dArr3[c];
        double dV3Dot5 = v3Dot(dArr[0], dArr[2]);
        dArr4[c] = dV3Dot5;
        double[] dArrV3Combine5 = v3Combine(dArr[2], dArr[0], 1.0d, -dV3Dot5);
        dArr[2] = dArrV3Combine5;
        double dV3Dot6 = v3Dot(dArr[c], dArrV3Combine5);
        dArr4[2] = dV3Dot6;
        double[] dArrV3Combine6 = v3Combine(dArr[2], dArr[c], 1.0d, -dV3Dot6);
        dArr[2] = dArrV3Combine6;
        double dV3Length6 = v3Length(dArrV3Combine6);
        dArr3[2] = dV3Length6;
        dArrV3Normalize = v3Normalize(dArr[2], dV3Length6);
        dArr[2] = dArrV3Normalize;
        double d11 = dArr4[c];
        double d12 = dArr3[2];
        dArr4[c] = d11 / d12;
        ...
        dArr6[0] = roundTo3Places((-Math.atan2(dArr18[c], dArr18[2])) * 57.29577951308232d);
        double[] dArr19 = dArr[2];
        double d13 = -dArr19[0];
        double d14 = dArr19[c];
        double d15 = dArr19[2];
        dArr6[c] = roundTo3Places((-Math.atan2(d13, Math.sqrt((d14 * d14) + (d15 * d15)))) * 57.29577951308232d);
        dArr6[2] = roundTo3Places((-Math.atan2(dArr[c][0], dArr[0][0])) * 57.29577951308232d);
    }
