    ...
    public final void dispatchOnModeChangeIfNeeded(boolean checkRectChange) {
        ...
        if ((this.targetRect.width() != 0 || this.targetRect.height() != 0) && !this.thresholdRect.isEmpty()) {
            ...
            if (rectsOverlap(this.targetRect, this.thresholdRect)) {
                ...
                double d = this.prerenderRatio;
                if (d > 0.0d) {
                    this.thresholdRect.inset((int) (((double) (-iWidth)) * d), (int) (((double) (-iHeight)) * d));
                ...
                if (zRectsOverlap) {
                    ...
                    virtualViewMode = this.mode;
                    if (virtualViewMode != null) {
                        double d2 = this.hysteresisRatio;
                        if (d2 > 0.0d) {
                            this.thresholdRect.inset((int) (((double) (-iWidth)) * d2), (int) (((double) (-iHeight)) * d2));
                            if (!rectsOverlap(this.targetRect, this.thresholdRect)) {
                                virtualViewMode = VirtualViewMode.Hidden;
                                this.thresholdRect.setEmpty();
                            } else if (getDebugLogEnabled()) {
                                if (ReactVirtualViewKt.IS_DEBUG_BUILD) {
                                    FLog.d("ReactVirtualView:dispatchOnModeChangeIfNeeded", ((Object) ("hysteresis, mode=" + virtualViewMode)) + " [" + getId() + "][" + getNativeId$ReactAndroid_release() + "]");
                                } else {
                                    FLog.w("ReactVirtualView:dispatchOnModeChangeIfNeeded", ((Object) ("hysteresis, mode=" + virtualViewMode)) + " [" + getId() + "][" + getNativeId$ReactAndroid_release() + "]");
                                }
                            }
                        } else {
                            virtualViewMode = VirtualViewMode.Hidden;
