...
public class DevServerHelper {
    ...
    private final String getInspectorDeviceId() {
        ...
        String string = Settings.Secure.getString(this.applicationContext.getContentResolver(), DeviceIdStorage.ANDROID_ID_SHARED_PREFS_KEY);
    ...
    private final String createBundleURL(String mainModuleID, BundleType type, String host, boolean modulesOnly, boolean runModule) {
        ...
        for (Map.Entry<String, String> entry : packagerConnectionSettings.updatePackagerOptions(packagerConnectionSettings.getAdditionalOptionsForPackager()).entrySet()) {
            ...
            if (value.length() != 0) {
                sb.append(MsalUtils.QUERY_STRING_DELIMITER + key + SimpleComparison.EQUAL_TO_OPERATION + Uri.encode(value));
            }
        }
        StringCompanionObject stringCompanionObject = StringCompanionObject.INSTANCE;
        String str = String.format(Locale.US, "http://%s/%s.%s?platform=android&dev=%s&lazy=%s&minify=%s&app=%s&modulesOnly=%s&runModule=%s", Arrays.copyOf(new Object[]{host, mainModuleID, type.getTypeID(), Boolean.valueOf(devMode), Boolean.valueOf(devMode), Boolean.valueOf(getJSMinifyMode()), this.packageName, modulesOnly ? TelemetryEventStrings.Value.TRUE : "false", runModule ? TelemetryEventStrings.Value.TRUE : "false"}, 9));
        Intrinsics.checkNotNullExpressionValue(str, "format(...)");
