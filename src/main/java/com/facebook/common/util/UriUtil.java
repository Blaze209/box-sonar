...
public class UriUtil {
    ...
    public static String getRealPathFromUri(ContentResolver contentResolver, Uri uri) {
        ...
        if (isLocalContentUri(uri)) {
            ...
            Cursor cursorQuery = MAMContentResolverManagement.query(contentResolver, uri2, new String[]{getDataPathString(z)}, str, strArr, null);
            if (cursorQuery != null) {
                try {
    ...
    public static AssetFileDescriptor getAssetFileDescriptor(ContentResolver contentResolver, Uri uri) {
        if (isLocalContentUri(uri)) {
            try {
                return MAMContentResolverManagement.openAssetFileDescriptor(contentResolver, uri, "r");
