...
public final class DbConnectionPoolMetrics {
    ...
    public static DbConnectionPoolMetrics create(OpenTelemetry openTelemetry, String str, String str2) {
        ...
        String strFindVersion = EmbeddedInstrumentationProperties.findVersion(str);
        if (strFindVersion != null) {
            meterBuilder.setInstrumentationVersion(strFindVersion);
        }
        return new DbConnectionPoolMetrics(meterBuilder.build(), Attributes.of(POOL_NAME, str2));
    }
    ...
    DbConnectionPoolMetrics(Meter meter, Attributes attributes) {
        ...
        this.usedConnectionsAttributes = builder.put(attributeKey, STATE_USED).build();
        this.idleConnectionsAttributes = attributes.toBuilder().put(attributeKey, STATE_IDLE).build();
    }
