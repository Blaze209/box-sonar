...
public final class ContextPropagationDebug {
    ...
    public static void debugContextLeakIfEnabled() {
        ...
        if (isThreadPropagationDebuggerEnabled() && (contextCurrent = Context.current()) != Context.root()) {
            Logger logger2 = logger;
            logger2.severe("Unexpected non-root current context found when extracting remote context!");
            Span spanFromContextOrNull = Span.fromContextOrNull(contextCurrent);
            if (spanFromContextOrNull != null) {
                logger2.log(Level.SEVERE, "It contains this span: {0}", spanFromContextOrNull);
            }
            debugContextPropagation(contextCurrent);
