...
public final class HttpServerMetrics implements OperationListener {
    ...
    public static /* synthetic */ HttpServerMetrics $r8$lambda$4f80X6eRCNvLsmBBrBkh9ukE714(Meter meter) {
    ...
    public static OperationMetrics get() {
        return new OperationMetrics() { // from class: io.opentelemetry.instrumentation.api.instrumenter.http.HttpServerMetrics$$ExternalSyntheticLambda0
            ...
            public final OperationListener create(Meter meter) {
                return HttpServerMetrics.$r8$lambda$4f80X6eRCNvLsmBBrBkh9ukE714(meter);
            }
        };
    }
    ...
    public void onEnd(Context context, Attributes attributes, long j) {
        ...
        Attributes attributesApplyServerDurationAndSizeView = TemporaryMetricsView.applyServerDurationAndSizeView(state.startAttributes(), attributes);
        this.duration.record((j - state.startTimeNanos()) / NANOS_PER_MS, attributesApplyServerDurationAndSizeView, context);
        Long l = (Long) getAttribute(SemanticAttributes.HTTP_REQUEST_CONTENT_LENGTH, attributes, state.startAttributes());
        if (l != null) {
            this.requestSize.record(l.longValue(), attributesApplyServerDurationAndSizeView);
        }
        Long l2 = (Long) getAttribute(SemanticAttributes.HTTP_RESPONSE_CONTENT_LENGTH, attributes, state.startAttributes());
        if (l2 != null) {
            this.responseSize.record(l2.longValue(), attributesApplyServerDurationAndSizeView);
        }
    }
