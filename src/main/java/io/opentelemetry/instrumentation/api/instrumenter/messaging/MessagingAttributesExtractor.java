...
public final class MessagingAttributesExtractor<REQUEST, RESPONSE> implements AttributesExtractor<REQUEST, RESPONSE>, SpanKeyProvider {
    ...
    MessagingAttributesExtractor(MessagingAttributesGetter<REQUEST, RESPONSE> messagingAttributesGetter, MessageOperation messageOperation, List<String> list) {
    ...
    public void onEnd(AttributesBuilder attributesBuilder, Context context, REQUEST request, @Nullable RESPONSE response, @Nullable Throwable th) {
        ...
        for (String str : this.capturedHeaders) {
            List<String> listHeader = this.getter.header(request, str);
            if (!listHeader.isEmpty()) {
                AttributesExtractorUtil.internalSet(attributesBuilder, CapturedMessageHeadersUtil.attributeKey(str), listHeader);
            }
        }
    }

    /* JADX INFO: renamed from: io.opentelemetry.instrumentation.api.instrumenter.messaging.MessagingAttributesExtractor$1, reason: invalid class name */
