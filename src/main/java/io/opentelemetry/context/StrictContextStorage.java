    ...
    final class StrictScope implements Scope {
        ...
        public void close() {
            ...
            for (int i = 0; i < stackTrace.length; i++) {
                ...
                if (stackTraceElement.getClassName().equals(StrictScope.class.getName()) && stackTraceElement.getMethodName().equals(HeaderElements.CLOSE)) {
                    int i2 = i + 2;
                    int i3 = i + 1;
    ...
    static class PendingScopes extends WeakConcurrentMap<Scope, CallerStackTrace> {
        ...
        public void run() {
            while (!Thread.interrupted()) {
                try {
                    CallerStackTrace callerStackTraceRemove = this.map.remove(remove());
