    ...
    public void subscribe(Subscriber<? super R>[] subscribers) {
        ...
        if (validate(subscriberArrOnSubscribe)) {
            ...
            for (int i = 0; i < length; i++) {
                try {
                    subscriberArr[i] = new ParallelReduceSubscriber(subscriberArrOnSubscribe[i], Objects.requireNonNull(this.initialSupplier.get(), "The initialSupplier returned a null value"), this.reducer);
                } catch (Throwable th) {
