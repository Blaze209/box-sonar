    ...
    public void subscribe(Subscriber<? super C>[] subscribers) {
        ...
        if (validate(subscriberArrOnSubscribe)) {
            ...
            for (int i = 0; i < length; i++) {
                try {
                    subscriberArr[i] = new ParallelCollectSubscriber(subscriberArrOnSubscribe[i], Objects.requireNonNull(this.initialCollection.get(), "The initialSupplier returned a null value"), this.collector);
                } catch (Throwable th) {
