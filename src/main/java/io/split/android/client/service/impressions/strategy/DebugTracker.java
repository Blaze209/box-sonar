...
class DebugTracker implements PeriodicTracker {
    ...
    DebugTracker(ImpressionsObserver impressionsObserver, RecorderSyncHelper<KeyImpression> impressionsSyncHelper, SplitTaskExecutor taskExecutor, ImpressionsTaskFactory taskFactory, RetryBackoffCounterTimer retryTimer, int impressionsRefreshRate) {
        SplitTaskExecutionListener splitTaskExecutionListener = new SplitTaskExecutionListener() { // from class: io.split.android.client.service.impressions.strategy.DebugTracker.1
            ...
            public void taskExecuted(SplitTaskExecutionInfo taskInfo) {
                if (taskInfo.getStatus() == SplitTaskExecutionStatus.ERROR && Boolean.TRUE.equals(taskInfo.getBoolValue(SplitTaskExecutionInfo.DO_NOT_RETRY))) {
                    DebugTracker.this.mIsSynchronizing.compareAndSet(true, false);
                    DebugTracker.this.stopPeriodicRecording();
                }
            }
        };
