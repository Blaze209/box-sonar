...
public class TelemetryStatsRecorderTask implements SplitTask {
    ...
    public TelemetryStatsRecorderTask(HttpRecorder<Stats> telemetryStatsRecorder, TelemetryStatsProvider telemetryStatsProvider, TelemetryRuntimeProducer telemetryRuntimeProducer) {
        this.mTelemetryStatsRecorder = (HttpRecorder) Utils.checkNotNull(telemetryStatsRecorder);
        this.mTelemetryStatsProvider = (TelemetryStatsProvider) Utils.checkNotNull(telemetryStatsProvider);
    ...
    public SplitTaskExecutionInfo execute() {
        ...
        try {
            try {
                ...
                splitTaskExecutionInfoError = SplitTaskExecutionInfo.success(SplitTaskType.TELEMETRY_STATS_TASK);
                ...
                if (HttpStatus.isNotRetryable(HttpStatus.fromCode(e.getHttpStatus()))) {
                    splitTaskExecutionInfoError = SplitTaskExecutionInfo.error(SplitTaskType.TELEMETRY_STATS_TASK, Collections.singletonMap(SplitTaskExecutionInfo.DO_NOT_RETRY, true));
                } else {
                    splitTaskExecutionInfoError = SplitTaskExecutionInfo.error(SplitTaskType.TELEMETRY_STATS_TASK);
                }
            }
