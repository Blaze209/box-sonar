...
abstract class UnicodeEscaper {
    ...
    private static final ThreadLocal<char[]> DEST_TL = new ThreadLocal<char[]>() { // from class: io.split.android.client.network.UnicodeEscaper.1
    ...
    protected final String escapeSlow(String s, int index) {
        int length = s.length();
        char[] cArrCharBufferFromThreadLocal = charBufferFromThreadLocal();
        int i = 0;
        int length2 = 0;
        while (index < length) {
