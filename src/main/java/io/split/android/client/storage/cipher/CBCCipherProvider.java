...
public class CBCCipherProvider implements CipherProvider {
    ...
    public CBCCipherProvider(String apiKey) {
        this.mKey = new KeyManager(apiKey).getKey();
        byte[] bArr = new byte[16];
        System.arraycopy(apiKey.getBytes(), 0, bArr, 0, 16);
    ...
    private Cipher getInitializedCipher(int encryptMode) {
        try {
            Cipher cipherAcquire = this.mCipherPool.acquire();
            cipherAcquire.init(encryptMode, this.mKey, this.mIvParameterSpec);
            return cipherAcquire;
