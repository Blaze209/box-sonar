...
abstract class BufferNextMessage<S> implements SpanWithSizeConsumer<S> {
    ...
    /* JADX INFO: renamed from: zipkin2.reporter.BufferNextMessage$1, reason: invalid class name */
    ...
    static <S> BufferNextMessage<S> create(Encoding encoding, int i, long j) {
    ...
    static final class BufferNextJsonMessage<S> extends BufferNextMessage<S> {
        ...
        void resetMessageSizeInBytes() {
            ...
            this.messageSizeInBytes = size + 1;
    ...
    public boolean offer(S s, int i) {
        byte b;
        int iMessageSizeInBytes = messageSizeInBytes(i);
        int i2 = this.maxBytes;
        if (iMessageSizeInBytes < i2) {
            b = -1;
        } else {
            b = iMessageSizeInBytes == i2 ? (byte) 0 : (byte) 1;
        }
        if (b > 0) {
        ...
        addSpanToBuffer(s, i);
        this.messageSizeInBytes = iMessageSizeInBytes;
        if (b == 0) {
            this.bufferFull = true;
        }
        return true;
    ...
    void drain(SpanWithSizeConsumer<S> spanWithSizeConsumer) {
