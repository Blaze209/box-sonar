...
public final class InMemoryReporterMetrics implements ReporterMetrics {
    ...
    public long messagesDropped() {
        Iterator<AtomicLong> it = this.messagesDropped.values().iterator();
        long jLongValue = 0;
        while (it.hasNext()) {
    ...
    static <K> void increment(ConcurrentHashMap<K, AtomicLong> concurrentHashMap, K k, int i) {
        ...
        AtomicLong atomicLongPutIfAbsent = concurrentHashMap.get(k);
        if (atomicLongPutIfAbsent == null && (atomicLongPutIfAbsent = concurrentHashMap.putIfAbsent(k, new AtomicLong(i))) == null) {
            return;
        }
        do {
            j = atomicLongPutIfAbsent.get();
        } while (!atomicLongPutIfAbsent.compareAndSet(j, ((long) i) + j));
    }

    private void update(MetricKey metricKey, int i) {
        AtomicLong atomicLongPutIfAbsent = this.metrics.get(metricKey);
        if (atomicLongPutIfAbsent == null && (atomicLongPutIfAbsent = this.metrics.putIfAbsent(metricKey, new AtomicLong(i))) == null) {
            return;
        }
        atomicLongPutIfAbsent.set(i);
    }
}
