...
public final class ValueClassAwareCallerKt {
    ...
    private static final List<String> getMfvcUnboxMethods$getUnboxMethodNameSuffixes(SimpleType simpleType) {
        ...
        ClassifierDescriptor classifierDescriptorMo16080getDeclarationDescriptor = simpleType.getConstructor().mo16080getDeclarationDescriptor();
        ...
        MultiFieldValueClassRepresentation<SimpleType> multiFieldValueClassRepresentation = DescriptorUtilsKt.getMultiFieldValueClassRepresentation((ClassDescriptor) classifierDescriptorMo16080getDeclarationDescriptor);
        ...
        for (Pair pair : underlyingPropertyNamesToTypes) {
            ...
            if (mfvcUnboxMethods$getUnboxMethodNameSuffixes == null) {
                ...
                while (it.hasNext()) {
                    arrayList2.add(name.getIdentifier() + Soundex.SILENT_MARKER + ((String) it.next()));
                }
    ...
    public static final Class<?> toInlineClass(KotlinType kotlinType) {
        ...
        Class<?> inlineClass = toInlineClass(kotlinType.getConstructor().mo16080getDeclarationDescriptor());
        if (inlineClass == null) {
            return null;
        }
