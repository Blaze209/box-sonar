        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(rigidTypeMarkerPop)) {
                ...
                if (none == null) {
                    ...
                    while (it.hasNext()) {
                        RigidTypeMarker rigidTypeMarkerMo16084transformType = none.mo16084transformType(typeCheckerState, it.next());
                        if ((typeSystemContext.isClassType(rigidTypeMarkerMo16084transformType) && !typeSystemContext.isMarkedNullable(rigidTypeMarkerMo16084transformType)) || typeSystemContext.isDefinitelyNotNullType(rigidTypeMarkerMo16084transformType)) {
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(rigidTypeMarkerPop)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(rigidTypeMarkerPop)).iterator();
                    while (it.hasNext()) {
                        RigidTypeMarker rigidTypeMarkerMo16084transformType = supertypesPolicy.mo16084transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, rigidTypeMarkerMo16084transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(rigidTypeMarkerMo16084transformType);
