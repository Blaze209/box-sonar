...
public final class LongSaturatedMathKt {
    ...
    public static final long m16264saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM16195toLongimpl = Duration.m16195toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m16263checkInfiniteSumDefinedPjuGub4(j, j2, jM16195toLongimpl);
        }
        if (((jM16195toLongimpl - 1) | 1) == Long.MAX_VALUE) {
            return m16265saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM16195toLongimpl;
        if (((j ^ j3) & (jM16195toLongimpl ^ j3)) < 0) {
    ...
    private static final long m16263checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m16181isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m16265saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM16159divUwyO8pc = Duration.m16159divUwyO8pc(j2, 2);
        long jM16195toLongimpl = Duration.m16195toLongimpl(jM16159divUwyO8pc, durationUnit);
        return (1 | (jM16195toLongimpl - 1)) == Long.MAX_VALUE ? jM16195toLongimpl : m16264saturatingAddNuflL3o(m16264saturatingAddNuflL3o(j, durationUnit, jM16159divUwyO8pc), durationUnit, Duration.m16184minusLRDsOJo(j2, jM16159divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m16250getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m16248getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m16200unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m16185plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m16200unaryMinusUwyO8pc(infinityOfSign(j3));
        }
